...
# MUDA: Porta padrão do Hugging Face Spaces
ENV PORT="7860" 
...
# MUDA: Expondo a porta 7860
EXPOSE 7860 

# Activate virtual environment and run the application with Gunicorn
# MUDA: Bindando o Gunicorn à porta 7860
CMD ["sh", "-c", "exec poetry run gunicorn mediaflow_proxy.main:app -w 4 -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:7860 --timeout 120 --max-requests 500 --max-requests-jitter 200 --access-logfile - --error-logfile - --log-level info --forwarded-allow-ips \"${FORWARDED_ALLOW_IPS:-127.0.0.1}\""]
